...
# pdfmain.sh
Calls other files from pdfmain directory to process PDFs, these files do not self-delete. Copy pdfmain.sh to PDF working directory and execute there. Non-deleting scripts need to be in /home/(user)/Documents/pdf-batch-scripts/pdfmain \
Dependencies:\
`sudo apt install pdftk`\
`sudo apt install imagemagick`\
`sudo apt install poppler-utils`\
